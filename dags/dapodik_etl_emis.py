...
dag = DAG(
    dag_id='dapodik_emis_etl',
    default_args=default_args,
    start_date=datetime(2024, 2, 9),
    end_date=datetime(2024, 6, 30),  # Menghentikan DAG pada tanggal 30 Juni 2024
    schedule_interval='0 2 * * 1',  # Menjalankan otomatis setiap senin jam 9 pagi (GMT+7)
    catchup=False,
    tags=['dapodik'],
) 
...
def etl(endpoint, ti):
    ...
    rows_npsn = sekolah.get_records("SELECT npsn FROM dapodik_emis.sekolah where bentuk_pendidikan IN ('RA','SPK TK','MI','SPK SD','MTs','SPK SMP','MA','SPK SMA')")
...
def create_pipeline(token_, endpoint, dag_, audit_):
    ...
    create_table = PostgresOperator(
        ...
        sql='dapodik_emis/sql/'+table_name+'_schema.sql',
        dag=dag_
    )
...
create_table_sekolah = PostgresOperator(
    ...
    postgres_conn_id='local-postgresql',
    sql='dapodik_emis/sql/sekolah_schema.sql',
    dag=dag
)
