...
dag = DAG(
    dag_id='dapodik_meranti_etl',
    default_args=default_args,
    start_date=datetime(2024, 2, 10),
    end_date=datetime(2024, 6, 30),  # Menghentikan DAG pada tanggal 30 Juni 2024
    schedule_interval='30 2 * * 5',  # Menjalankan otomatis setiap jum'at jam 9.30 pagi (GMT+7)
    catchup=False,
    tags=['dapodik'],
) 
...
def etl(endpoint, ti):
    ...
    rows_npsn = sekolah.get_records("SELECT npsn FROM dapodik_meranti.sekolah WHERE bentuk_pendidikan IN ('SD', 'SMP', 'KB', 'TK', 'TPA', 'SKB', 'PKBM', 'SPS', 'Kursus') AND kabupaten IN ('Kab. Kepulauan Meranti')")
...
def create_pipeline(token_, endpoint, dag_, audit_):
    ...
    create_table = PostgresOperator(
        ...
        sql='dapodik_meranti/sql/'+table_name+'_schema.sql',
        dag=dag_
    )
...
create_table_sekolah = PostgresOperator(
    ...
    postgres_conn_id='local-postgresql',
    sql='dapodik_meranti/sql/sekolah_schema.sql',
    dag=dag
)
