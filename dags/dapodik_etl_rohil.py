...
dag = DAG(
    dag_id='dapodik_rohil_etl',
    default_args=default_args,
    start_date=datetime(2024, 2, 9),
    end_date=datetime(2024, 6, 30),  # Menghentikan DAG pada tanggal 30 Juni 2024
    schedule_interval='30 0 * * 3',  # Menjalankan otomatis setiap rabu jam 7.30 pagi (GMT+7)
    catchup=False,
    tags=['dapodik'],
) 
...
def etl(endpoint, ti):
    ...
    rows_npsn = sekolah.get_records("SELECT npsn FROM dapodik_rohil.sekolah WHERE bentuk_pendidikan IN ('SD', 'SMP', 'KB', 'TK', 'TPA', 'SKB', 'PKBM', 'SPS', 'Kursus') AND kabupaten IN ('Kab. Rokan Hilir')")
...
def create_pipeline(token_, endpoint, dag_, audit_):
    ...
    create_table = PostgresOperator(
        ...
        sql='dapodik_rohil/sql/'+table_name+'_schema.sql',
        dag=dag_
    )
...
create_table_sekolah = PostgresOperator(
    ...
    postgres_conn_id='local-postgresql',
    sql='dapodik_rohil/sql/sekolah_schema.sql',
    dag=dag
)
