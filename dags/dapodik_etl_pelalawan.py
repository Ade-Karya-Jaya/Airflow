...
dag = DAG(
    dag_id='dapodik_pelalawan_etl',
    default_args=default_args,
    start_date=datetime(2024, 2, 9),
    end_date=datetime(2024, 6, 30),  # Menghentikan DAG pada tanggal 30 Juni 2024
    schedule_interval='0 23 * * 4',  # Menjalankan otomatis setiap jum'at jam 6 pagi (GMT+7)
    catchup=False,
    tags=['dapodik'],
) 
...
def etl_sekolah(ti):
    ...
    table_name = 'sekolah'
    table_schema = 'dapodik_pelalawan'
...
def etl(endpoint, ti):
    ...
    rows_npsn = sekolah.get_records("SELECT npsn FROM dapodik_pelalawan.sekolah WHERE bentuk_pendidikan IN ('SD', 'SMP', 'KB', 'TK', 'TPA', 'SKB', 'PKBM', 'SPS', 'Kursus') AND kabupaten IN ('Kab. Pelalawan')")
...
def create_pipeline(token_, endpoint, dag_, audit_):
    ...
    create_table = PostgresOperator(
        ...
        sql='dapodik_pelalawan/sql/'+table_name+'_schema.sql',
        dag=dag_
    )
...
create_table_sekolah = PostgresOperator(
    ...
    postgres_conn_id='local-postgresql',
    sql='dapodik_pelalawan/sql/sekolah_schema.sql',
    dag=dag
)
